...
# ── AC1: _guarded_broadcast catches the exception and emits a warning ─────────
...
class TestGuardedBroadcastCatchesException:
    def test_guarded_broadcast_does_not_raise(self):
        ...
        asyncio.run(_bc._guarded_broadcast(_raising_broadcast, {"type": "board_invalidated", "project": "owner/repo"}))
    ...
    def test_guarded_broadcast_calls_logger_warning_on_failure(self):
        ...
        assert mock_warn.called, "logger.warning must be called when broadcast raises"
    ...
    def test_guarded_broadcast_does_not_warn_on_success(self):
        ...
        assert not mock_warn.called, "logger.warning must not be called on success"
...
class TestGuardedBroadcastWarningContext:
    def test_warning_includes_project_name(self):
        ...
        assert project in full_message, (
            f"project name '{project}' must appear in warning; got: {full_message!r}"
        )
...
class TestInvalidateBoardUsesGuard:
    def test_broadcast_exception_does_not_propagate_from_invalidate_board(self):
        ...
        async def run():
            with (
                patch("board_cache._bc", side_effect=RuntimeError("boom")) if False else _noop_ctx(),
            ):
                # Patch broadcast via the import path board_cache resolves
                pass

        # Simpler: patch logs_service.broadcast to raise, then call invalidate_board
        # inside a running loop and drain tasks.
        async def _test():
            async def bad_broadcast(payload):
                raise RuntimeError("broadcast failed")

            with patch("board_cache._guarded_broadcast", wraps=_bc._guarded_broadcast):
                # Monkey-patch logs_service so the import inside invalidate_board
                # resolves to our raising coroutine.
                import types
                ...
                try:
                    with patch.object(_bc.logger, "warning") as mock_warn:
                        ...
                        await asyncio.sleep(0)
                    assert mock_warn.called, "logger.warning should fire when broadcast raises"
                finally:
                    if old is None:
    ...
    def test_invalidate_board_warning_includes_project_name_in_async_path(self):
        ...
        async def _test():
            ...
            assert project in full, f"project name must appear in warning; got: {full!r}"

        asyncio.run(_test())


# ── Context manager stub for patch noop ──────────────────────────────────────
