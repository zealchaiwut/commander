...
Follow-up sprint of five code-review cleanups — one API-response-shape fix, two dead-code/duplication removals, and two test-coverage upgrades — with no new user-facing features. **Orphaned cost block dropped from `/analytics/metrics` (#1871):** `_compute_analytics_metrics` in `apps/dashboard/startup.py` still computed a `cost` block (blended per-token pricing joined from `token_usage`, distributed across roles, plus a `rework_cost_annotation`) that no consumer read, so the whole `MODEL_PRICE_MAP` table, the `status_tokens_in/out` accumulators, the `token_usage` join, and the `cost` key in the response were removed; the endpoint now returns `first_pass_rate`, `rework_rate`, `avg_duration`, `throughput`, `most_reworked`, and `by_sprint` only. The endpoint has no detailed response-shape section in `docs/features/api.md` (summary-table row only), so the API doc is unchanged. **Symlink guard runs on the pre-resolve path (#1878):** the docs-service path check in `apps/dashboard/routers/docs_service.py` tested `candidate.is_symlink()`, but `candidate` is already `Path.resolve()`d (which follows every symlink), so the check was always `False` and dead. It now tests `(clone_root / rel_path).is_symlink()` — the un-resolved request path — so a symlinked doc path is rejected as intended. **Duplicate #1825 test file removed (#1922):** `tests/test_remove_venv_symlink__1825.py` duplicated `tests/test_1825__remove_venv_symlink.py`; the redundant copy was deleted and a guard test (`test_1922__cleanup_duplicate_1825.py`) plus an all-AC verifier (`test_1922__verify_all_ac.py`) added. **Bulk-move node test exercises the real function (#1924):** the `_smgmtMoveModalPickBulk` handler was extracted from `project.html` inline JS into `apps/dashboard/static/src/sprint-board/bulk-move.js` (exported and re-exposed on `globalThis` via `index.js`, bundle rebuilt) so the `bulk-move-new-sprint-clear-selection` node test drives the actual handler through a fetch spy instead of re-implementing the fix (per CLAUDE.md #1746). **Duplicate `token_usage` query removed (#1967):** `_compute_cost` in `scripts/export_hermes_report.py` called `_query_token_usage(conn)` twice (once per pricing branch); the query is now run once at the top and the rows reused. Test-and-cleanup sprint; no new DB tables, columns, or endpoints, so `SCHEMA.md` is unchanged.

- [#1871](https://github.com/zealchaiwut/commander/issues/1871) [follow-up] Drop orphaned cost block from /analytics/metrics response — 2026-07-21
- [#1878](https://github.com/zealchaiwut/commander/issues/1878) [follow-up] Symlink guard in docs_service runs on resolved path (dead check) — 2026-07-21
- [#1922](https://github.com/zealchaiwut/commander/issues/1922) [follow-up] Duplicate #1825 test files: test_remove_venv_symlink__1825.py overlaps test_1825__remove_venv_symlink.py — 2026-07-21
- [#1924](https://github.com/zealchaiwut/commander/issues/1924) [follow-up] #1760 node test simulates the fix instead of exercising _smgmtMoveModalPickBulk — 2026-07-21
- [#1967](https://github.com/zealchaiwut/commander/issues/1967) [follow-up] Avoid duplicate token_usage query in _compute_cost — 2026-07-21
