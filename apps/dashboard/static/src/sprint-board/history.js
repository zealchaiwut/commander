...
function _histIssueSucceeded(iss, sprint) {
  return (
    _histIssueChip(iss, { binary: _histSprintShowsBinaryIssues(sprint) })
      .cls === "merged"
  );
}
...
function _histChildCardHtml(s, group, opts) {
  ...
  const fromLine =
    !isLineageParent && _histIsChild(s.label)
...
function _histChildSprintsStillRunning(group) {
  ...
  return (group.children || []).some((s) =>
    active.has((s.lifecycle_state || "").toLowerCase()),
  );
}
...
function _histGroupHtml(group) {
  ...
  if (children.length) {
    ...
    const groupCls =
      "hist-sprint-group" + (_histExpanded.has(baseLbl) ? "" : " collapsed");
    ...
    const childHtml = children
      .map((c) => _histChildCardHtml(c, group))
      .join("");
