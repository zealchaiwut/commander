...
- `GET /openapi.json` — the full, always-current OpenAPI schema generated by
  ...
  so their response shapes appear in the schema: `POST /api/sprints/{label}/dispatch`,
  `POST /api/sprints/{label}/overnight`, `GET /api/sprints/{sprint_label}/live`,
  `GET /api/dev-report`, `POST /api/tickets/draft`, `POST /api/tickets/create`,
  `GET /api/failures`, `GET /api/board` (models in `hermes_models.py`).
  **Deleted:** `POST /api/sprints/run` — use dispatch / overnight instead.
  Reviewed subset: `docs/api/overnight.yaml`.
