...
---
Wire impeccable skills into BA design requirements and coder frontend work so UI tickets carry concrete design rules end to end. impeccable ships design-rule skill packs via `npx impeccable skills install`, which scaffold into `.github/skills/impeccable/` and are loaded by agents through `node .github/skills/impeccable/scripts/context.mjs`. (1) Ensure the skills are installed/scaffolded as part of project setup (extend .commander/setup.sh or the design-docs guard so `.github/skills/impeccable/` exists). (2) In .claude/agents/ba.md, when a ticket has frontend/UI scope, the BA loads the impeccable design skills and bakes the relevant rules (spacing scale, contrast, component naming, responsive breakpoints) into the acceptance criteria so design expectations are explicit, not implied. (3) In .claude/agents/coder.md, when implementing frontend, the coder loads the same impeccable skills so its output follows the design system and passes the existing impeccable `detect` design gate on the first try. Acceptance: a new UI ticket's AC references concrete design rules sourced from impeccable skills, and the coder loads those skills before writing frontend code; the design gate pass rate on frontend tickets improves because rules are followed up front rather than caught at the gate.
```
