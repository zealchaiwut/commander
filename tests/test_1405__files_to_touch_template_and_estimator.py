...
STARTUP_PY = REPO_ROOT / "apps" / "dashboard" / "startup.py"
BULK_TICKETS_PY = REPO_ROOT / "apps" / "dashboard" / "routers" / "bulk_tickets.py"
...
def test_server_ba_prompt_includes_files_to_touch_section():
    """AC3: BA prompts list '## Files to touch' as a draft section."""
    for path in (STARTUP_PY, BULK_TICKETS_PY):
        content = path.read_text()
        assert "Files to touch" in content, (
            f"{path.name} BA prompts must include '## Files to touch' as a draft section"
        )
