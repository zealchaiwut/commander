...
class TestAC6NoRegression:
    ...
    def test_pytest_timeout_is_configured(self):
        """pytest.ini or setup.cfg must set a default timeout so no test hangs."""
        ini_path = REPO_ROOT / "pytest.ini"
        setup_cfg = REPO_ROOT / "setup.cfg"
        found = False
        for path in (ini_path, setup_cfg):
            if path.exists() and "timeout" in path.read_text():
                found = True
                break
        assert found, "pytest timeout must be configured in pytest.ini or setup.cfg"
