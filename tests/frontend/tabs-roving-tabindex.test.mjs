...
function assertAllMinusOne(map, except) {
  for (const [key, val] of Object.entries(map)) {
    ...
    assert.equal(
      val,
      -1,
      `Expected ${key} to be -1 when ${except} owns the active tab`,
    );
  }
}
...
test("AC3: roadmap active → planning trigger gets 0, others get -1", () => {
  ...
  assert.equal(
    map["planning"],
    0,
    "planning should be 0 when roadmap is active",
  );
...
test("AC3: advisor active → planning trigger gets 0, others get -1", () => {
  ...
  assert.equal(
    map["planning"],
    0,
    "planning should be 0 when advisor is active",
  );
  assertAllMinusOne(map, "planning");
...
test("AC4: returns all -1 when onGlobalSettings=true (strip is hidden)", () => {
  for (const tab of ALL_TABS) {
    ...
    assert.equal(
      zeros.length,
      0,
      `Expected no tabIndex=0 in global-settings mode, got: ${JSON.stringify(map)}`,
    );
  }
});
