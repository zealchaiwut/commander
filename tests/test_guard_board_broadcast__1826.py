"""Tests for issue #1826: Guard board_invalidated broadcast.

Tests the _guarded_broadcast helper that wraps SSE broadcasts with
exception handling. Issue #1826 adds a guard against unhandled asyncio
task exceptions when broadcasting board_invalidated events. Previously,
if broadcast() raised, it would surface as an unretrieved-task warning
at GC time with no context. Now it's caught via logger.warning.
"""
import asyncio
import logging
import sys
...
async def test_guarded_broadcast__success(caplog, board_cache):
    ...
    assert not any(
        "broadcast failed" in record.message for record in caplog.records
    )


@pytest.mark.asyncio
async def test_guarded_broadcast__catches_exception(caplog, board_cache):
    """AC: _guarded_broadcast catches exceptions and logs via warning."""
    caplog.set_level(logging.WARNING)
    ...
    assert any(
        "board_invalidated broadcast failed" in record.message
        and "owner/repo" in record.message
        for record in caplog.records
    ), "Expected warning not found. Logs: " + str(
        [r.message for r in caplog.records]
    )


@pytest.mark.asyncio
async def test_guarded_broadcast__logs_project_and_exc(caplog, board_cache):
    """AC: _guarded_broadcast logs include project name and exception info."""
    caplog.set_level(logging.WARNING)
    ...
    assert len(warning_records) > 0, (
        f"No warning logged. Records: {caplog.records}"
    )
...
@pytest.mark.asyncio
async def test_guarded_broadcast__handles_any_exc_type(caplog, board_cache):
    """AC: _guarded_broadcast handles any Exception subclass."""
    caplog.set_level(logging.WARNING)
...
def test_invalidate_board__broadcasts_when_no_exception(board_cache):
    """AC: invalidate_board schedules broadcast via create_task in async."""
    called_args = []
    ...
    with patch.object(
        board_cache, "_guarded_broadcast", side_effect=mock_guarded_broadcast
    ):
        mock_bc = AsyncMock()
        fake_mod = type("module", (), {"broadcast": mock_bc})()
        with patch.dict("sys.modules", {"logs_service": fake_mod}):
            async def run_test():
