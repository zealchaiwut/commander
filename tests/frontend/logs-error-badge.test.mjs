...
test("AC2: agent_finished with status=error is an error event", () => {
  assert.ok(evlIsErrorEvent({ type: "agent_finished", detail: { status: "error" } }));
});

test("AC2: agent_finished with status=timed_out is an error event", () => {
  assert.ok(evlIsErrorEvent({ type: "agent_finished", detail: { status: "timed_out" } }));
});

test("AC2: agent_finished with status=done is NOT an error event", () => {
  assert.ok(!evlIsErrorEvent({ type: "agent_finished", detail: { status: "done" } }));
});
...
test("AC3: URL includes since= when sinceTs is provided", () => {
  ...
  assert.ok(url.includes(encodeURIComponent(ts)), "URL must include encoded timestamp");
});

test("AC3: URL falls back to limit=200 when sinceTs is null (first visit)", () => {
  const url = buildEvlFetchUrl("myrepo", null);
  assert.ok(!url.includes("since="), `URL must NOT contain since= on first visit, got: ${url}`);
  assert.ok(url.includes("limit=200"), `URL must contain limit=200, got: ${url}`);
});

test("AC3: URL falls back to limit=200 when sinceTs is empty string", () => {
  const url = buildEvlFetchUrl("myrepo", "");
  assert.ok(!url.includes("since="), `URL must NOT contain since= for empty string, got: ${url}`);
  assert.ok(url.includes("limit=200"), `URL must contain limit=200, got: ${url}`);
});

test("AC3: URL targets the correct project slug", () => {
  const url = buildEvlFetchUrl("commander", "2026-01-01T00:00:00Z");
  assert.ok(url.includes("/api/projects/commander/events"), `URL must target /api/projects/commander/events, got: ${url}`);
});

test("AC3: slug is URI-encoded in the URL", () => {
  const url = buildEvlFetchUrl("my project", "2026-01-01T00:00:00Z");
  assert.ok(url.includes("my%20project"), `Slug must be URI-encoded, got: ${url}`);
});
...
test("logsErrorBadgeKey returns per-slug key", () => {
  assert.equal(logsErrorBadgeKey("commander"), "commander_logs_last_visit_commander");
  assert.equal(logsErrorBadgeKey("perf-coach"), "commander_logs_last_visit_perf-coach");
});
...
test("logsWriteLastVisit stores ISO timestamp; logsReadLastVisit retrieves it", () => {
  ...
  assert.ok(!isNaN(Date.parse(stored)), `stored value must be a valid ISO date, got: ${stored}`);
});
