...
| `POST` | `/api/sprints/{sprint_label}/complete-step?project=` | Complete a single step of the finish flow. When the immediate parent branch was merged and pruned, the step now walks up the sprint lineage to retarget the next surviving ancestor branch (or `develop`) instead of silently no-op'ing and stranding the child's commits. Refuses a base-sprint complete-step with `409` while any child sprint still has a live branch — running (`Child sprint … is still running`) or with unmerged commits (`… has unmerged commits …`) — and skips closing tickets that belong to such unmerged children (issue #1934) |
