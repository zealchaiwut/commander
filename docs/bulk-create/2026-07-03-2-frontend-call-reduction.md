...
---
Drive the live sprint board from the existing SSE stream instead of the 2s snapshot poll. SEQUENCING: run AFTER sprint-102 (board aggregate) and sprint-103 (/api/running snapshot) — this ticket pushes deltas on top of those snapshots; do not duplicate their batched-request work here.

While a sprint runs, _smgmtLivePollTick (project.html:19258, setInterval 2000) fires /api/sprint-status plus /api/sprints/{label}/live per running sprint every 2s — ~60 calls/min for one sprint. A per-sprint SSE stream already exists (/api/sprints/{label}/live/stream, routers/sprint_live.py:726, consumed at project.html:27672) but only the logs inspector uses it; the board re-polls snapshot data alongside it. The inspector additionally polls /logs/tail or dispatch-log every 5s (:18273/:18307, plus an early-run fallback fetch :18326) duplicating the same stream's log_line events. Additionally, when max_coder_slots>1 the lanes view fires _smgmtWorkerLogFetch (project.html:17333, called from _smgmtLanesUpdate:17406) — /api/sprints/{label}/issue/{num}/log?tail_lines=40 per active coder inside the same 2s tick, adding N_coders × 30 calls/min.

Fix (SSE scope only — the batched-request fallback previously described here is superseded by sprint-103's /api/running snapshot): subscribe the board to the live/stream EventSource — have the server push the snapshot payload (or a delta) the board currently polls, either as a new snapshot event type on the existing stream or by reusing current events; use the sprint-103 /api/running snapshot as the reconnect-fallback payload with a slow poll (≥15s) only. Wire the inspector's log pane AND the lanes view's per-worker log tails to the same stream's log_line events (keyed by issue number) and drop both the 5s inspector poll and the per-worker tail fetches, with a slow poll fallback.

AC must cover: with one running sprint and the board open, steady-state polling is ≤4 calls/min; multi-slot runs add zero per-worker log polling while the stream is connected; board metrics still update within 5s of an agent event; inspector log lines still stream; stream disconnect falls back to the /api/running slow poll and recovers.
---
