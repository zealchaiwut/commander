...
# ---------------------------------------------------------------------------
# Local rolling backup (issue #1901)
# ---------------------------------------------------------------------------
...
def backup_db_local(
    ...
    """Take an online-backup copy of db_path into backup_dir as a timestamped .bak file.

    Uses SQLite's online-backup API for a consistent copy under concurrent writes.
    After writing, prunes old copies so at most n_keep remain. Returns the new path.
    """
    ...
    backup_file = backup_dir / f"{db_path.stem}.db.{ts}.bak"
    ...
    # Prune oldest copies beyond n_keep
    existing = sorted(backup_dir.glob("*.bak"), key=lambda p: p.name, reverse=True)
    for old in existing[n_keep:]:
        try:
            old.unlink()
        except OSError:
            pass

    log.info("backup_local: wrote %s (keeping %d)", backup_file.name, n_keep)
    return backup_file


def list_local_backups(backup_dir: Path) -> list:
    """Return .bak files in backup_dir sorted newest-first (by filename)."""
    d = Path(backup_dir)
    if not d.exists():
        return []
...
def _run_local_backup_in_thread() -> None:
    """Run a local rolling backup in the calling thread (called from timer)."""
    db_path = _resolve_db_path()
    if not db_path:
        log.debug("backup_local: no DB found, skipping")
        return
    try:
        dashboard_dir = Path(__file__).resolve().parent.parent.parent / "apps" / "dashboard"
        backup_dir = dashboard_dir / ".commander" / "db-backups"
        backup_db_local(db_path, backup_dir)
    except Exception as exc:
        log.error("backup_local: failed — %s", exc)


def _local_schedule_next() -> None:
    """Self-rescheduling local backup timer tick."""
...
def start_local_backup_scheduler() -> None:
    """Start the hourly self-rescheduling local-backup timer.
    ...
    log.info("backup_local: 1-hour scheduler started")
