...
def _make_called_process_error(
    stderr="gh: error: not authenticated", returncode=1
):
...
def test_fetch_issue_gh_failure_no_raw_traceback():
    """_fetch_issue must not let CalledProcessError propagate."""
    mod = _load_lint_module()
    with patch.object(
        mod.subprocess, "run",
        side_effect=_make_called_process_error(),
    ):
        try:
            mod._fetch_issue(99, "owner/repo")
        except subprocess.CalledProcessError:
            pytest.fail(
                "CalledProcessError escaped _fetch_issue"
                " — raw traceback would be shown"
            )
        except SystemExit:
...
def test_fetch_issue_gh_failure_exits_with_code_1():
    ...
    with patch.object(
        mod.subprocess, "run",
        side_effect=_make_called_process_error(returncode=1),
    ):
        with pytest.raises(SystemExit) as exc_info:
            mod._fetch_issue(99, "owner/repo")
...
def test_fetch_issue_gh_failure_prints_gh_stderr(capsys):
    """gh stderr must appear in error output so user knows what failed."""
    mod = _load_lint_module()
    err = _make_called_process_error(stderr="gh: HTTP 401: Bad credentials")
...
def test_fetch_issue_gh_failure_prints_gh_stderr_not_found(capsys):
    ...
    mod = _load_lint_module()
    err = _make_called_process_error(stderr="gh: HTTP 404: Not Found")
    with patch.object(mod.subprocess, "run", side_effect=err):
        with pytest.raises(SystemExit):
...
def test_fetch_issue_gh_failure_prints_readable_message(capsys):
    """Human-readable error must appear in stderr, not raw Python."""
    mod = _load_lint_module()
    with patch.object(
        mod.subprocess, "run",
        side_effect=_make_called_process_error(),
...
def test_fetch_issue_gh_failure_message_mentions_issue_number(capsys):
    """Error message must include the issue number."""
    mod = _load_lint_module()
    with patch.object(
        mod.subprocess, "run",
        side_effect=_make_called_process_error(),
...
def test_fetch_issue_success_returns_dict():
    """Success path still returns a dict with expected keys."""
    mod = _load_lint_module()
