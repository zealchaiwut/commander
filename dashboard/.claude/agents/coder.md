...
All subsequent git operations and script calls run from this directory. Do **not** run git commands from `~/commander/dashboard/` — that is the dashboard service, not the project root.

### Step 2 — Create the feature branch

```bash
python3 ~/commander/dashboard/scripts/start_feature.py --issue <N>
```
...
```bash
python3 ~/commander/dashboard/scripts/update_ticket.py --issue <N> --status sit
```

### Step 9 — Comment

```bash
python3 ~/commander/dashboard/scripts/comment_ticket.py \
  --issue <N> \
