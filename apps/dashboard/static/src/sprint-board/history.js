...
function _histIssueChip(iss) {
  ...
  if (
    iss.failure_reason ||
    (iss.agent_status || "").toLowerCase() === "failed"
  ) {
...
function _histHeadStatsHtml(s) {
  ...
  const agentSecs =
    stats && stats.has_runs && stats.agent_total_seconds != null
  ...
  return (
    '<span class="hist-head-stats">' +
    parts
      .map((p) => '<span class="hist-head-stat">' + escHtml(p) + "</span>")
      .join("") +
    "</span>"
  );
}
...
function _histIssueLogUrl(s, issueNum) {
  ...
  return (
    base + sep + "issue=" + encodeURIComponent(String(issueNum)) + "&view=raw"
  );
}
...
export function _histEstBarMini(s) {
  ...
  const accPct =
    acc <= 1 ? Math.round(acc * 100) : Math.round(Math.min(100, 100 / acc));
  const tone =
    acc > 1.25 ? "var(--red)" : acc > 1.0 ? "var(--amber)" : "var(--green)";
...
function _histIssueRowHtml(iss, isChild, s) {
  ...
  const title = titleText
    ? `<span class="iss-title">${escHtml(titleText)}</span>`
    : "";
...
function _histPartialChildrenHtml(s) {
  ...
  const links = children
    .map((c) => {
      const lbl = escHtml(c);
      const display =
        typeof sprintLabelDisplay === "function" ? sprintLabelDisplay(c) : c;
...
export function _histMetricsHtml(s) {
  ...
  if (s.duration != null) {
    badges.push(
      `<span class="hist-badge"><i class="ti ti-clock"></i> ${escHtml(_histFmtSecs(s.duration))}</span>`,
    );
  }
  if (s.tokens != null) {
    badges.push(
      `<span class="hist-badge"><i class="ti ti-coin"></i> ${escHtml(_histFmtTokens(s.tokens))} tok</span>`,
    );
  }
  const state = (s.lifecycle_state || "").toLowerCase();
  const issues = Array.isArray(s.issues) ? s.issues : [];
  if (state === "partial_finished") {
    ...
    if (n > 0) {
      badges.push(
        `<span class="hist-badge"><i class="ti ti-arrows-split"></i> ${n} child sprint${n !== 1 ? "s" : ""}</span>`,
      );
    }
  } else {
    badges.push(
      `<span class="hist-badge"><i class="ti ti-ticket"></i> ${issues.length} tickets</span>`,
    );
  }
...
export function _histStateChip(state, sprint) {
  ...
  const er =
    sprint && sprint.end_reason ? String(sprint.end_reason).toLowerCase() : "";
  const displayState =
    s === "needs_rework" &&
    (er === "natural" || er === "merge_sprint") &&
    sprint &&
    !_histSprintFailed(sprint)
      ? "ready_to_merge"
      : s;
  const map = {
  ...
  const reason =
    sprint && sprint.end_reason && _histSprintFailed(sprint)
...
function _histAutoExpandRecent(groups) {
  ...
  for (let i = 0; i < groups.length; i++) {
    ...
    if (
      children.length &&
      baseLbl &&
      !_histCollapseDefaultsApplied.has(baseLbl)
    ) {
      _histCollapseDefaultsApplied.add(baseLbl);
      const st = (
        (g.baseSprint && g.baseSprint.lifecycle_state) ||
        ""
      ).toLowerCase();
...
function _histStatsHtml(s) {
  ...
  const wall =
    hasRuns && stats.wall_seconds != null ? stats.wall_seconds : s.duration;
  const tokens =
    hasRuns && stats.total_tokens != null ? stats.total_tokens : s.tokens;
  ...
  if (hasRuns) {
    if (stats.fix_round_count > 0) {
      const refs = (stats.fix_round_tickets || [])
        .map((n) => "#" + n)
        .join(", ");
    ...
    if (stats.slowest_ticket) {
      chips.push(
        _histStatChip(
          ...
          "slowest",
          "#" +
            stats.slowest_ticket.ticket +
            " · " +
            _histFmtSecs(stats.slowest_ticket.seconds),
        ),
      );
    }
    ...
    if (
      stats.coder_backend_split &&
      stats.coder_backend_split.cline_count > 0
    ) {
      const bs = stats.coder_backend_split;
      const parts = [];
      if (bs.cline_count > 0)
        parts.push(
          "cline: " + bs.cline_count + " · " + _histFmtSecs(bs.cline_seconds),
        );
      if (bs.claude_code_count > 0)
        parts.push(
          "claude-code: " +
            bs.claude_code_count +
            " · " +
            _histFmtSecs(bs.claude_code_seconds),
        );
      if (parts.length)
    ...
    if (sprintFailed && stats.crash) {
      const failed = (
        Array.isArray(s.failed_tickets) ? s.failed_tickets : []
      ...
      chips.push(
        _histStatChip(
          "ti-alert-triangle",
          "crash",
          "#" + stats.crash.ticket + " · " + crashAgent + tail,
          "stat-chip--crash",
        ),
      );
    }
...
function _histPostSprintHtml(s) {
  ...
  if (doc) {
    ...
    if (doc.status === "skipped") {
      ...
      body = (doc.files_touched || [])
        .map((f) => `<code class="ps-file">${escHtml(String(f))}</code>`)
        .join("");
      if (doc.commit_sha) {
  ...
  if (rev) {
    ...
    if (rev.status === "skipped") {
      ...
      if (counts.length)
        body += `<div class="ps-meta">${escHtml(counts.join(" · "))}</div>`;
      if (rev.comment_url) {
...
export function _histHeadHintsHtml(s, expanded) {
  if (expanded) {
    return (
      _histPostSprintChipHtml(s) +
      _histReconcileChipHtml(s) +
      _histStaleChipHtml(s)
    );
  }
  let html = "";
  const r = s.reconciliation;
...
function _histMetricsElapsedLabel(s, stats) {
  const hasRuns = !!(stats && stats.has_runs);
  const secs =
    hasRuns && stats.agent_total_seconds != null
      ? stats.agent_total_seconds
      : s.duration;
  if (secs == null) return "";
...
function _histAgentTimeBarHtml(stats) {
  ...
  const inner = segs
    .map((seg) => {
      ...
      return (
        `<span class="hist-agent-bar-seg ${seg.cls}" style="width:${seg.pct}%"` +
...
function _histAgentBreakdownHtml(stats) {
  ...
  return `<div class="hist-agent-breakdown">${rows
    .map(
      (r) =>
        `<span class="hist-agent-at"><span class="hist-swatch ${r.cls}"></span>` +
...
function _histMetricsChipsHtml(s, stats) {
  ...
  return chips.length
    ? `<div class="hist-metric-chips">${chips.join("")}</div>`
    : "";
}
    ...
    .map((t) => {
      ...
      const segs = (t.segments || [])
        .map((seg) => {
          ...
          const agentCls =
            seg.agent === "tester" ? "hist-tl-tester" : "hist-tl-coder";
...
function _histCardShowsDoneSummary(s) {
  ...
  if (state === "needs_rework" || state === "partial_finished") {
    const unfinished = issues.filter(
      (i) => (i.state || "").toLowerCase() !== "merged",
    );
  ...
  return (
    state === "ready_to_merge" || state === "completed" || state === "running"
  );
}
...
function _histChildCardHtml(s) {
  ...
  const displayState =
    state === "needs_rework" &&
    s.end_reason &&
    (String(s.end_reason).toLowerCase() === "natural" ||
      String(s.end_reason).toLowerCase() === "merge_sprint") &&
    !_histSprintFailed(s)
      ? "ready_to_merge"
      : state;
  ...
  return (
    `<div class="${cls.join(" ")}" data-label="${lbl}">
    ...
    (fromLine
      ? ` <span class="hist-child-from">${escHtml(fromLine)}</span>`
      : "") +
    `</span>
        ${_histStateChip(s.lifecycle_state, s)}
        ${_histHeadStatsHtml(s)}
...
function _histLooseEndBandHtml(s) {
  ...
  if (r && Array.isArray(r.checks)) {
    ...
    if (staleCheck) {
      const offenders = Array.isArray(staleCheck.tickets)
        ? staleCheck.tickets
        : [];
      const looseN = r.checks.filter((c) => !c.ok).length || 1;
...
function _histCardHtml(s, opts) {
  ...
  const display =
    typeof sprintLabelDisplay === "function"
      ? sprintLabelDisplay(s.label)
      : s.label || "";
  const chev = expanded ? "ti-chevron-down" : "ti-chevron-right";
  const lbl = escHtml(s.label || "");
  ...
  const secondaryLinks = _histSecondaryLinksHtml(s);
  const headRight =
    secondaryLinks || deleteBtn || bulkBtn || recoveryBtn
...
function _histGroupSprints(sprints) {
  ...
  const _groupUnsettled = (g) =>
    [g.baseSprint, ...(g.children || [])]
      .filter(Boolean)
...
function _histChildSprintsAllCompleted(group) {
  ...
  return children.every((s, i) => {
    ...
    return children
      .slice(i + 1)
      .some((later) =>
        settled.has((later.lifecycle_state || "").toLowerCase()),
      );
  });
}
...
function _histGroupHtml(group) {
  ...
  if (children.length) {
    const baseLbl =
      group.baseLabel || (group.baseSprint && group.baseSprint.label) || "";
    const groupExpanded = baseLbl ? !_histGroupCollapsed.has(baseLbl) : true;
    const groupCls = groupExpanded
      ? "hist-sprint-group"
      : "hist-sprint-group collapsed";
    ...
    const parentOwnIssues =
      group.baseSprint && Array.isArray(group.baseSprint.issues)
        ? group.baseSprint.issues
        : [];
    const parentBody =
      groupExpanded && parentOwnIssues.length
...
function _histTicketsDone(s) {
  const issues = Array.isArray(s.issues) ? s.issues : [];
  return issues.filter((i) => (i.state || "").toLowerCase() === "merged")
    .length;
}
...
function _histFoldHtml(fold) {
  ...
  const range =
    fold.from != null && fold.to != null
      ? fold.from === fold.to
        ? "Sprint " + fold.from
        : "Sprints " + fold.from + "–" + fold.to
...
export function _histRenderLedger(sprints) {
  ...
  el.innerHTML =
    _histLegendHtml() + _histToolbarHtml() + recentHtml + foldsHtml;
}
...
export async function _histLoadLedger(repo, opts) {
  ...
  const fresh =
    !force && hasCache && Date.now() - _histLedgerCacheAt < _HIST_LEDGER_TTL_MS;

  if (fresh) {
    if (
      !background &&
      el &&
      !el.querySelector(".hist-card, .hist-sprint-group, .hist-fold")
    ) {
      _histRenderLedger(_histLedgerData);
    }
    return;
  ...
  const loadPromise = (async () => {
    try {
      ...
      if (sresp && sresp.ok) {
        try {
          ...
          if (!isNaN(ttlMin) && ttlMin > 0)
            _HIST_LEDGER_TTL_MS = ttlMin * 60000;
        } catch (_) {
          /* keep current fold size / ttl */
        }
      }
      if (!resp.ok) {
      ...
      const histOpen = document
        .getElementById("smgmt-subview-history")
