...
- `GET /openapi.json` — the full, always-current OpenAPI schema generated by
  FastAPI (every route below plus request/response models).
...
| Method | Path | Description |
|---|---|---|
| `GET` | `/api/projects/{owner}/{repo_name}/sprints/{label}/finish-preview` | Preview what "finish sprint" would do |
| `POST` | `/api/projects/{owner}/{repo_name}/sprints/{label}/finish` | Finish a sprint (merge, close, summarize) |
| `POST` | `/api/projects/{owner}/{repo_name}/sprints/{label}/finish-bg` | Start finish-sprint as a background job; returns `{started, job_key}` |
| `GET` | `/api/projects/{owner}/{repo_name}/sprints/{label}/finish-stream` | SSE stream of finish-sprint progress; resends current snapshot on reconnect |
| `GET` | `/api/projects/{owner}/{repo_name}/sprints/{label}/bulk-complete-preview` | Dry-run preview of bulk-complete |
| `POST` | `/api/projects/{owner}/{repo_name}/sprints/{label}/bulk-complete` | Bulk-complete all eligible tickets in a sprint |
| `POST` | `/api/projects/{owner}/{repo_name}/sprints/{label}/complete-step` | Complete a single step of the finish flow |
| `GET` | `/api/projects/{owner}/{repo_name}/sprints/{label}/conflict-status` | Merge-conflict status for a sprint's branches |
| `GET` | `/api/sprints/{label}/reconcile-preview` | Dry-run: GitHub-vs-DB diff + post-sprint checks for one sprint. No writes; requires `?project=` |
| `POST` | `/api/sprints/{label}/reconcile` | Apply reconcile for one sprint: correct DB lifecycle + local state from GitHub truth. Never writes to GitHub |
