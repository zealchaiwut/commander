...
BASE_URL = os.environ.get("UAT_BASE_URL") or "http://localhost:" + os.environ.get(
    "UAT_PORT", ""
)
...
def test_dead_if_ok_guard__removed_guard(client):
    ...
    with open(
        os.path.join(
            os.path.dirname(__file__), "../apps/dashboard/routers/sprint_history.py"
        ),
        "r",
    ) as f:
        content = f.read()
    ...
    # Find the HTTPException raise (may be on separate lines after formatting)
    ...
    assert exception_line_idx is not None, "HTTPException raise not found in function"
...
def test_dead_if_ok_guard__exception_path_intact(client):
    ...
    assert "raise HTTPException" in content and "500" in content, (
        "HTTPException(500) missing"
    )
    assert '"error": result.get("error")' in content, (
        "error field missing in exception detail"
    )
    ...
    assert '"compensation": result.get("compensation")' in content, (
        "compensation field missing"
    )
...
def test_dead_if_ok_guard__success_path_unconditional(client):
    ...
    assert exception_start is not None, "raise HTTPException not found"
    ...
    for i, line in enumerate(after_exception_lines):
        if line.strip().startswith("try:"):
            ...
            assert leading_spaces == 4, (
                f"try block not at function body level (indent={leading_spaces})"
            )
            break
...
def test_dead_if_ok_guard__no_syntax_errors(client):
    ...
    try:
        ...
        assert hasattr(sprint_history, "split_xl_apply"), (
            "split_xl_apply function not found"
        )
