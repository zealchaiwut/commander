...
test("getEnvironment: two calls produce one fetch", async () => {
  ...
  const counts = installFetchSpy({
    "/api/environment": { environment: "prd", port: 8000 },
  });

  const [r1, r2] = await Promise.all([getEnvironment(), getEnvironment()]);

  assert.equal(
    counts["/api/environment"],
    1,
    "must fetch /api/environment exactly once",
  );
...
test("getEnvironment: second sequential call returns cached data without re-fetching", async () => {
  _resetCacheForTesting();
  const counts = installFetchSpy({
    "/api/environment": { environment: "uat" },
  });
...
test("getVersion: two calls produce one fetch", async () => {
  ...
  assert.equal(
    counts["/api/version"],
    1,
    "must fetch /api/version exactly once",
  );
...
test("getSettings: two parallel calls produce one fetch", async () => {
  ...
  assert.equal(
    counts["/api/settings"],
    1,
    "must fetch /api/settings exactly once",
  );
  assert.deepEqual(r1, r2);
});
...
test("getSettings: three sequential calls produce one fetch", async () => {
  _resetCacheForTesting();
  const counts = installFetchSpy({
    "/api/settings": { history_cache_ttl_min: 10 },
  });
...
test("invalidateSettings: next getSettings() call re-fetches", async () => {
  ...
  globalThis.fetch = async (url) => {
    if (String(url) === "/api/settings") {
      ...
      return {
        ok: true,
        json: async () => ({ history_cache_ttl_min: callN * 5 }),
      };
    }
  ...
  assert.equal(
    second.history_cache_ttl_min,
    10,
    "must re-fetch after invalidation",
  );
...
test("invalidateSettings: does not affect environment or version caches", async () => {
  ...
  assert.equal(
    counts["/api/environment"],
    1,
    "environment cache must be untouched",
  );
  assert.equal(counts["/api/version"], 1, "version cache must be untouched");
  assert.equal(
    counts["/api/settings"],
    2,
    "settings must be fetched twice (once + after invalidation)",
  );
});
...
test("cache is NOT persisted to localStorage", () => {
  ...
  const trap = new Proxy(
    {},
    {
      get(_, k) {
        throw new Error(
          `localStorage.${String(k)} accessed — must not use localStorage`,
        );
      },
      set(_, k) {
        throw new Error(
          `localStorage.${String(k)} set — must not use localStorage`,
        );
      },
    },
  );
...
test("cache is NOT persisted to sessionStorage", () => {
  const trap = new Proxy(
    {},
    {
      get(_, k) {
        throw new Error(
          `sessionStorage.${String(k)} accessed — must not use sessionStorage`,
        );
      },
      set(_, k) {
        throw new Error(
          `sessionStorage.${String(k)} set — must not use sessionStorage`,
        );
      },
    },
  );
