...
class TestRouterFileExists:
    """AC-1: routers/project_branches.py exists with APIRouter + correct prefix."""
    ...
    def test_router_has_correct_prefix(self):
        """AC-1: APIRouter must have prefix /api/projects/{owner}/{repo}/branches."""
        source = ROUTER_FILE.read_text()
...
class TestGetStaleBranchesRoute:
    """AC-2: GET /api/projects/{owner}/{repo}/branches/stale handled by new router."""
    ...
    def test_get_stale_branches_returns_200(self):
        ...
        with patch("subprocess.run") as mock_run, \
            ...
            resp = client.get("/api/projects/owner/testrepo/branches/stale")
        assert resp.status_code == 200
        assert isinstance(resp.json(), list)

    def test_get_stale_branches_returns_matching_branches(self):
        """AC-4: Response schema matches pre-refactor behavior — sorted list of branch names."""
        client, srv = _get_test_client()
        merged = json.dumps([{"headRefName": "feature/100-foo"}])
        existing = "feature/100-foo\ndevelop\nmaster\n"
        with patch("subprocess.run") as mock_run, \
...
class TestDeleteBranchRoute:
    """AC-3: DELETE /api/projects/{owner}/{repo}/branches/{branch} in new router."""

    def test_delete_branch_function_in_router(self):
        """AC-3: delete_project_branch function defined in project_branches.py."""
        source = ROUTER_FILE.read_text()
    ...
    def test_delete_branch_success_returns_200(self):
        ...
        with patch("subprocess.run") as mock_run, \
            ...
            resp = client.delete("/api/projects/owner/testrepo/branches/feature/100-foo")
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_delete_protected_branch_returns_400(self):
        ...
        with patch.object(srv.projects_module, "load_projects", return_value=FAKE_PROJECT):
            resp = client.delete("/api/projects/owner/testrepo/branches/develop")
    ...
    def test_delete_master_returns_400(self):
        ...
        client, srv = _get_test_client()
        with patch.object(srv.projects_module, "load_projects", return_value=FAKE_PROJECT):
            resp = client.delete("/api/projects/owner/testrepo/branches/master")
        assert resp.status_code == 400
    ...
    def test_delete_non_feat_sprint_returns_400(self):
        ...
        client, srv = _get_test_client()
        with patch.object(srv.projects_module, "load_projects", return_value=FAKE_PROJECT):
            resp = client.delete("/api/projects/owner/testrepo/branches/hotfix/bug-123")
        assert resp.status_code == 400
    ...
    def test_delete_gh_failure_returns_500(self):
        ...
        with patch("subprocess.run") as mock_run, \
             patch.object(srv.projects_module, "load_projects", return_value=FAKE_PROJECT):
            mock_run.return_value = _make_proc("", returncode=1, stderr="not found")
            resp = client.delete("/api/projects/owner/testrepo/branches/feature/100-foo")
...
class TestServerPyNoDirectRoutes:
    """AC-5/AC-6: server.py only includes the new router, defines no direct routes."""
    ...
    def test_server_py_has_no_stale_route_decorator(self):
        ...
        assert '@app.get("/api/projects/{owner}/{repo_name}/branches/stale")' not in source, (
            "server.py still defines @app.get('.../branches/stale') directly"
        )

    def test_server_py_has_no_delete_branch_route_decorator(self):
        """AC-5: @app.delete('.../branches/{branch:path}') not present in server.py."""
        source = SERVER_PY.read_text()
        assert '@app.delete("/api/projects/{owner}/{repo_name}/branches/{branch:path}"' not in source, (
            "server.py still defines @app.delete('.../branches/{branch:path}') directly"
        )

    def test_server_py_includes_project_branches_router(self):
        """AC-5: server.py must include the project_branches_router via include_router."""
        source = SERVER_PY.read_text()
    ...
    def test_server_py_does_not_define_get_stale_branches(self):
        """AC-5: get_stale_branches function not defined (decorated) in server.py."""
        source = SERVER_PY.read_text()
        ...
        for i, line in enumerate(lines):
            if "def get_stale_branches" in line:
                ...
                if i > 0 and "@app." in lines[i - 1]:
                    pytest.fail(
                        "server.py defines get_stale_branches as an @app route handler"
                    )

    def test_server_py_does_not_define_delete_project_branch(self):
        """AC-5: delete_project_branch not defined as @app route in server.py."""
        source = SERVER_PY.read_text()
        lines = source.splitlines()
        for i, line in enumerate(lines):
            if "def delete_project_branch" in line:
                if i > 0 and "@app." in lines[i - 1]:
                    pytest.fail(
                        "server.py defines delete_project_branch as an @app route handler"
                    )
