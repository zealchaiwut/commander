"""Tests for issue #1844 — Home AI summary enrichment must respect brief_disabled() gate.
...
AC3  Behavioral TestClient test: with the disable flag set, GET /api/brief/daily
...
_FAKE_ARTIFACT = {
    ...
    "brief": {
        ...
        "kpis": {"sprints_shipped": 0, "tickets_done": 0, "in_progress": 0, "needs_your_call": 0},
        "decisions": [],
...
def _get_brief_router_module():
    """Return the brief module object as used by the running server (routers.brief)."""
    # DASHBOARD_DIR is on sys.path, so server imports routers.brief (not apps.dashboard.routers.brief)
...
def test_ac1_summary_skipped_when_brief_disabled():
    """_enrich_home_artifact must not call get_or_create_project_summary when disabled."""
    ...
    assert summary_mock.call_count == 0, (
        "get_or_create_project_summary was called despite brief_disabled()=True"
    )
    # The project entry must carry an empty briefSummary (deterministic fallback).
...
def test_ac2_summary_called_when_brief_enabled():
    """_enrich_home_artifact must still call get_or_create_project_summary when enabled."""
    brief_router = _get_brief_router_module()
    ...
    assert summary_mock.call_count == 1, (
        "get_or_create_project_summary should be called when brief_disabled()=False"
    )
...
def test_ac3_testclient_brief_disabled_does_not_invoke_summary():
    """GET /api/brief/daily with brief_disabled=True must not call the summary generator."""
    ...
    with patch.object(brief_router, "config") as mock_cfg, \
        ...
        mock_cfg.brief_disabled.return_value = True
        mock_bs.get_or_create_project_summary = summary_mock
        mock_ba.get_or_create_home_artifact.return_value = copy.deepcopy(_FAKE_ARTIFACT)
        mock_proj.load_projects.return_value = _FAKE_PROJECTS
    ...
    assert summary_mock.call_count == 0, (
        f"get_or_create_project_summary was called {summary_mock.call_count} time(s) "
        "despite brief_disabled()=True — gate is missing in _enrich_home_artifact"
    )
