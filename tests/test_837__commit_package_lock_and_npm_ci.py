...
def lock_data():
    assert PACKAGE_LOCK.exists(), "package-lock.json must exist at the repo root"
    return json.loads(_read(PACKAGE_LOCK))


@pytest.fixture(scope="module")
def ci_text():
    assert FRONTEND_BUILD_YML.exists(), ".github/workflows/frontend-build.yml must exist"
...
def test_837__package_lock_exists():
    """AC1: package-lock.json file must exist in the same directory as package.json."""
    assert PACKAGE_JSON.exists(), "package.json must exist at the repo root"
    assert PACKAGE_LOCK.exists(), "package-lock.json must be committed at the repo root"
    assert PACKAGE_LOCK.stat().st_size > 0, "package-lock.json must be non-empty"
...
def test_837__package_lock_contains_esbuild(lock_data):
    ...
    assert esbuild_key in packages, "package-lock.json must contain node_modules/esbuild"
...
def test_837__package_lock_contains_eslint(lock_data):
    ...
    assert eslint_key in packages, "package-lock.json must contain node_modules/eslint"
    version = packages[eslint_key].get("version", "")
    assert re.match(r"^\d+\.\d+\.\d+", version), \
        f"eslint version '{version}' must be a semver string"
    major = int(version.split(".")[0])
    assert major >= 9, f"eslint version {version} must satisfy ^9.0.0 (major >= 9)"


def test_837__package_lock_lockfile_version(lock_data):
    """AC2 (structural): package-lock.json must be a valid lockfile (version 1-3)."""
...
def test_837__ci_uses_npm_ci(ci_text):
    """AC3: the 'Install dependencies' step must run 'npm ci', not 'npm install'."""
...
def test_837__ci_no_npm_install(ci_text):
    ...
    for line in ci_text.splitlines():
        ...
        assert "npm install" not in stripped, \
            f"frontend-build.yml must not use 'npm install'; found: {line!r}"


# AC4 — paths filter retains package-lock.json and it now exists
def test_837__ci_paths_filter_includes_package_lock(ci_text):
    """AC4: the paths trigger in frontend-build.yml must include package-lock.json."""
...
def test_837__package_lock_makes_paths_filter_meaningful():
    """AC4 (file existence): package-lock.json must exist so the paths filter is non-trivial."""
    assert PACKAGE_LOCK.exists(), \
        "package-lock.json must be present in the repo so the paths filter is meaningful"
