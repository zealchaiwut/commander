"""Tests for issue #2262: DoR parser, preflight readiness, board gate.
...
AC4: Ticket missing AC flagged in warn mode, refused in block mode
...
from services.sprint_manager.ticket_spec import (  # noqa: E402
    parse_ticket_spec,
)
...
def test_ac1_parse_ticket_spec_empty_body_returns_empty_fields():
    """AC1: empty body returns empty values, never raises."""
...
def test_ac1_parse_ticket_spec_no_ac_section():
    """AC1: no AC heading yields empty acceptance_criteria list."""
    body = (
        "## Design Refs\n- figma.com/abc\n\n"
        "## UAT Test Steps\nDo the thing."
    )
...
def test_ac1_parse_ticket_spec_checklist_items_extracted():
    """AC1: checklist lines extracted correctly."""
    body = (
        "## Acceptance Criteria\n"
        "- [ ] First criterion\n"
        "- [x] Second criterion\n"
...
def _make_preflight_client(
    ...
    with (
        ...
        patch.object(
            srv,
            "_resolve_issue_estimate",
            side_effect=fake_resolve_estimate,
        ),
        patch.object(
            srv,
            "_effective_agent_models",
            side_effect=fake_effective_models,
        ),
        patch(
...
def test_ac2_preflight_returns_readiness_block_warn_mode(tmp_path):
    ...
    resp = _make_preflight_client(
        tmp_path, [_make_issue(1)], dor_mode="warn"
    )
...
def test_ac2_readiness_block_has_ready_and_not_ready_keys(tmp_path):
    """AC2: readiness has 'ready' list and 'not_ready' list."""
    resp = _make_preflight_client(
        tmp_path, [_make_issue(1)], dor_mode="warn"
    )
    assert resp.status_code == 200
...
def test_ac2_ready_ticket_in_ready_list(tmp_path):
    ...
    resp = _make_preflight_client(
        tmp_path, [_make_issue(42)], dor_mode="warn"
    )
    assert resp.status_code == 200
    body = resp.json()
    assert 42 in body["readiness"]["ready"]
    assert not any(
        e["number"] == 42 for e in body["readiness"]["not_ready"]
    )
...
def test_ac3_off_mode_omits_readiness_block(tmp_path):
    """AC3: definition_of_ready_mode=off → readiness key absent."""
    resp = _make_preflight_client(
        tmp_path, [_make_issue(1)], dor_mode="off"
    )
    assert resp.status_code == 200
    body = resp.json()
...
def test_ac4_warn_mode_ok_stays_true_for_not_ready_ticket(tmp_path):
    """AC4: warn mode flags ticket as not ready but keeps ok=True."""
...
def test_ac4_block_mode_refuses_when_ticket_missing_ac(tmp_path):
    """AC4: block mode → ok=False when ticket is missing AC."""
...
def test_ac3_block_mode_includes_readiness_block(tmp_path):
    """AC3: block mode → readiness block is present."""
...
def test_ac3_block_mode_ok_true_when_all_tickets_ready(tmp_path):
    ...
    resp = _make_preflight_client(
        tmp_path, [_make_issue(30)], dor_mode="block"
    )
    assert resp.status_code == 200
    body = resp.json()
...
def test_ac5_dor_mode_returned_in_response(tmp_path):
    """AC5: preflight response includes dor_mode field."""
...
def test_ac5_all_three_modes_produce_correct_ok_behavior(tmp_path):
    """AC5: off/warn=ok True, block=ok False when not-ready."""
    issue = _make_issue(99, has_ac=False)
    estimates = {99: {"estimated": True, "size": "S", "files": []}}

    resp_off = _make_preflight_client(
        tmp_path, [issue], dor_mode="off", estimates=estimates
    )
    resp_warn = _make_preflight_client(
        tmp_path, [issue], dor_mode="warn", estimates=estimates
    )
    resp_block = _make_preflight_client(
        tmp_path, [issue], dor_mode="block", estimates=estimates
    )

    assert resp_off.json()["ok"] is True, "off mode must not block"
    assert resp_warn.json()["ok"] is True, "warn mode must not block"
    assert resp_block.json()["ok"] is False, "block mode must refuse"
