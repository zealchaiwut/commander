...
- **impeccable skills** — design-rule skill packs (`npx impeccable skills install`,
  repo pbakaus/impeccable → `.github/skills/impeccable/`). Loadable by BA/coder
  for design guidance.
- **Skill vs plugin** — pipeline uses the file-based **skills install**, NOT the
  Claude Code **plugin** (`/plugin marketplace add pbakaus/impeccable`). Sprint
  agents run headless (`claude -p`), which do not auto-load plugins, so prompts
  must point at the installed skill files. The plugin is only for a human's
  interactive Claude Code session (separate from the sprint). For mock iteration
  on claude.ai web, neither installs directly — use claude.ai's native
  web-artifacts-builder / theme-factory, or build + upload the compiled skill.
...
---
Wire impeccable skills into BA design requirements and coder frontend work so UI tickets carry concrete design rules end to end, with an attached mock HTML as the visual source of truth. impeccable ships design-rule skill packs via `npx impeccable skills install` (repo: pbakaus/impeccable), which compile into file-based skills under `.github/skills/impeccable/` and are loaded by agents through `node .github/skills/impeccable/scripts/context.mjs`. IMPORTANT — use the file-based skills install, NOT the Claude Code plugin (`/plugin marketplace add pbakaus/impeccable`): the sprint dispatches coder and tester as headless `claude -p` subprocesses, and headless runs do not auto-load plugins or subagents (same reason sprint_manager injects personas manually). So the agent prompts must explicitly reference the installed skill files; the plugin is only useful for a human's interactive Claude Code session and is not part of the pipeline. (1) Ensure the skills are installed/scaffolded as part of project setup (extend .commander/setup.sh or the design-docs guard so `.github/skills/impeccable/` exists), and have the coder/tester dispatch prompts in services/sprint_manager/sprint_manager.py load the skill via context.mjs so the headless agents actually pick it up. (2) In .claude/agents/ba.md, when a ticket has frontend/UI scope: if a mock HTML file is attached to the issue (under references/issue-<N>/), the BA reads the mock and extracts its actual design tokens — colors, spacing, typography, component structure — cross-checks them against the impeccable skills, and bakes those concrete values into the acceptance criteria so the mock becomes the design contract; if no mock is attached, the BA falls back to the generic impeccable design rules (spacing scale, contrast, component naming, responsive breakpoints). Either way the AC must state explicit design expectations, not implied ones. (3) In .claude/agents/coder.md, when implementing frontend, the coder loads the same impeccable skills AND, when a mock HTML is attached, treats that mock as the pixel/visual target to reproduce, so its output matches the mock, follows the design system, and passes the existing impeccable `detect` design gate on the first try. Acceptance: for a UI ticket with an attached mock, the AC references concrete design tokens extracted from that mock and the coder reproduces the mock as the visual source of truth; for a UI ticket without a mock, the AC references impeccable design rules; in both cases the coder loads the skills before writing frontend code and the design gate pass rate on frontend tickets improves.
```
