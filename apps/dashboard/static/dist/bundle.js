(() => {
  ...
  async function smgmtRerunSprint(label) {
    ...
    if (confirmBtn) {
      ...
      confirmBtn.textContent = "Create sprint";
    }
    _rrOpen();
    try {
      ...
      if (confirmBtn)
        confirmBtn.textContent = `Create ${sprintLabelDisplay(_rrVersionedLabel)}`;
  ...
  async function _rrConfirm() {
    ...
    try {
      ...
      if (confirmBtn) {
        ...
        confirmBtn.textContent = _rrVersionedLabel ? `Create ${sprintLabelDisplay(_rrVersionedLabel)}` : "Create sprint";
      }
    }
  }
...
${data.errors.join("\n")}`);
  ...
  function _smgmtRender2(data) {
    ...
    const orderedLabels = order.length > 0 ? order.filter((l) => /^sprint-\d+(\.\d+)*$/.test(l)) : [...sprints].sort((a, b) => a - b).map((n) => `sprint-${n}`);
