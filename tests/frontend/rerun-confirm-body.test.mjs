...
function _installDom(checkedIssues) {
  const els = {
    "rr-confirm-btn": _makeEl({
      disabled: true,
      textContent: "Create sprint and run",
    }),
    "rr-content": _makeEl(),
    "rr-loading": _makeEl(),
    "rr-error": _makeEl(),
...
test("_rrConfirm: sends POST with valid body (ticket_numbers array + auto_run)", async () => {
  ...
  const fetchCalls = _installFetch(() =>
    _ok({
  ...
  assert.ok(
    call.url.includes("/api/sprints/sprint-106/rerun"),
    "URL targets rerun endpoint",
  );
  assert.ok(call.url.includes("project="), "URL has project param");

  const sentBody = JSON.parse(call.opts.body);
  assert.ok(
    Array.isArray(sentBody.ticket_numbers),
    "ticket_numbers must be an array",
  );
  assert.deepEqual(
    sentBody.ticket_numbers,
    [42, 99],
    "ticket_numbers contains checked issues",
  );
  assert.ok("auto_run" in sentBody, "body must include auto_run field");
  assert.equal(
    call.opts.headers["Content-Type"],
    "application/json",
...
test("_rrConfirm: shows error toast on 422 (not swallowing)", async () => {
  ...
  const errorToasts = toastCalls.filter(
    (t) =>
      t.msg.includes("body field required") || t.msg.includes("Re-run failed"),
  );
  assert.ok(errorToasts.length > 0, "error toast must fire on 422");
  assert.ok(
    errorToasts.some((t) => (t.level || "").includes("error")),
    "toast level must indicate error",
  );
});
...
test("_rrConfirm: shows toast with server detail from 422 response body", async () => {
  ...
  assert.ok(
    allMsgs.includes("Missing required field"),
    "toast must include server detail from 422 body",
  );
});
...
test("smgmtRerunSprint: shows warning toast when repo is null", async () => {
  ...
  const warn = toastCalls.find(
    (t) => t.level === "warning" || t.msg.toLowerCase().includes("no project"),
  );
