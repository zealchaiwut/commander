"""TDD tests for issue #1898: Hermes loop conflict auto-resolve and machine-readable 409.

Acceptance criteria:
- AC1: complete-step auto-resolves append-only conflicts in CHANGELOG.md / SCHEMA.md /
       models.py via union merge; overlapping conflicts are NOT auto-resolved.
- AC2: When a conflict is not auto-resolvable, endpoint returns 409 with
       {code: "merge_conflict_needs_human", files: [...], label: ...}.
- AC3: GET /conflict-status surfaces "blocked on conflict, human needed" per sprint.
- AC4: Tests confirm append-only auto-resolve succeeds, overlapping returns needs-human,
       and the loop-driver contract (skip + continue) is exercisable.
"""
...
class TestCompleteStepStructured409:
    ...
    def test_needs_human_conflict_returns_structured_409(self, client, monkeypatch):
    ...
    def test_non_conflict_failure_keeps_plain_409(self, client, monkeypatch):
...
class TestLoopDriverContract:
    ...
    def test_409_body_is_json_with_code_field(self, client, monkeypatch):
