...
| `POST` | `/api/sprints/{sprint_label}/tickets/{issue_number}/split/suggest` | Generate two draft child titles and AC scopes for splitting an oversized/parallelism-blocking ticket, via a lightweight BA call (issue #1423). Body `{project}`. Returns `{child1: {title, ac_scope}, child2: {title, ac_scope}, fallback}`; `fallback: true` with empty titles when the BA call is unavailable so the user can split manually |
| `POST` | `/api/sprints/{sprint_label}/tickets/{issue_number}/split/confirm` | Create two child GitHub issues from a split and handle the original (issue #1423). Body `{project, child1_title, child1_body?, child2_title, child2_body?, action}` where `action` is `remove` (drop original from sprint, keep open) or `close` (close original with a comment referencing the two children). Children inherit the parent's labels minus per-ticket lifecycle/size labels (`size-*`, `estimated`, `in-progress`) and each fires a background re-estimate. Returns `{child1_num, child2_num, ok}`; 400 when either child title is missing |
