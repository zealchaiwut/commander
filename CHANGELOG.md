...
Planning-board visibility and cross-project navigation, all landing on existing backend data. **Live cross-project status view (#2261):** `static/home.html` was rewritten from the nightly Dev Report into a live status view that consumes `/api/home` directly — per-project sprint state rendered on load instead of a stale generated report. **Definition of Ready — parser, preflight readiness, board gate (#2262):** `GET /api/sprints/{label}/preflight` now sets `ok: false` when `dor_mode == "block"` and any work ticket is `not_ready` (previously `ok` was hard-coded `true`); the `readiness` (`ready` / `not_ready` with per-ticket `missing` reasons) and `dor_mode` fields already returned inline continue to drive the board's ✓/✗ badges. Non-work tickets (`sprint-summary`, `docs`, `documentation`) are skipped; DoR stays advisory in `warn`/`off` modes. **Diagnostics reachable from the shell (#2263):** `diagnostics.html` now has a nav link from the current project shell so it is no longer an orphaned page. **Surface buried planning data (#2264):** the sprint board injects two server-computed panels inline on cards — a stale-estimate warning row (from `/preflight` `warnings.stale_estimates`, planning-state cards) and a collapsible per-ticket estimate-vs-actual panel (from `/api/sprints/{label}/estimate-vs-actual`, finished-sprint cards) — surfacing data that previously required opening a modal. **Project switcher in the header (#2291):** a keyboard-accessible, all-viewport `<select>` in the project header switches projects while preserving the active tab, so navigation no longer routes back through the home page. Bundle rebuilt and committed.

- [#2261](https://github.com/zealchaiwut/commander/issues/2261) Live cross-project status view to replace the nightly Dev Report — 2026-08-13
- [#2262](https://github.com/zealchaiwut/commander/issues/2262) Definition of Ready — parser, preflight readiness, board gate — 2026-08-13
- [#2263](https://github.com/zealchaiwut/commander/issues/2263) Make diagnostics.html reachable from the current shell — 2026-08-13
- [#2264](https://github.com/zealchaiwut/commander/issues/2264) Surface buried planning data already computed by the backend — 2026-08-13
- [#2291](https://github.com/zealchaiwut/commander/issues/2291) Project switcher in the project header so navigation does not route through home — 2026-08-13

## Sprint 1024.1

Dead-code removal pass — deleting five unused or superseded dashboard subsystems now that the platform is operator-driven. **Delete the Todos feature (#2255):** removed the per-project to-do scratchpad in favour of a hand-maintained `docs/todo.md` — the `/api/projects/{project}/todos*` CRUD, the `/api/todos` batch endpoint, attachments, the `project_todos` store (`todo_repo.py`, `todo_attachment_repo.py`, `ProjectTodo` model), and the `project-todo.js` panel UI. **Delete the docs-freshness subsystem (#2256):** removed `scripts/check_docs_freshness.py`, the `docs_freshness_warnings` table and its `db.py` helpers, and the `/api/docs-freshness/*` endpoints (`system_misc.py`). **Delete brief caching, LLM summary, and the daily-report job (#2257):** removed the cached Haiku brief-summary layer (`brief_summary.py`) and the persisted daily-artifact layer (`brief_artifact.py`, `brief_invalidation.py`) along with their `.../brief/summary` and `.../brief/daily` endpoints, plus the `generate_daily_report.py` launchd job (`com.commander.daily-report.plist`, `install_daily_report_launchd.sh`). The deterministic `/api/brief` and `/api/projects/{slug}/brief` assembly endpoints remain; the `brief_summaries` table is now orphaned (kept to avoid a migration) and `brief_artifacts` survives because `/api/dev-report` still uses it. **Delete the unused changelog API (#2258):** removed `changelog.py` / `changelog_service.py` and the `/api/projects/{slug}/changelog` endpoint. **Delete dead milestone surfaces, keep the selector (#2259):** removed the per-project milestone CRUD (`/api/projects/{slug}/milestones` GET/POST/PATCH/DELETE), the `/api/projects/{slug}/issues` mirror endpoint, and the milestone-progress indicator (`home_milestone.py`, `/api/home/milestone`); the cross-project selector (`GET /api/milestones`) is retained.

- [#2255](https://github.com/zealchaiwut/commander/issues/2255) Delete the Todos feature in favour of docs/todo.md — 2026-08-13
- [#2256](https://github.com/zealchaiwut/commander/issues/2256) Delete the docs-freshness subsystem — 2026-08-13
- [#2257](https://github.com/zealchaiwut/commander/issues/2257) Delete brief caching, LLM summary, and the daily-report job — 2026-08-13
- [#2258](https://github.com/zealchaiwut/commander/issues/2258) Delete the unused changelog API — 2026-08-13
- [#2259](https://github.com/zealchaiwut/commander/issues/2259) Delete dead milestone surfaces, keep the selector — 2026-08-13

## Sprint 1023

Final phase of the autonomous-dispatch shrink (`commander-shrink-2026-08`): deleted the sprint orchestrator itself now that the operator drives sprints by hand, and moved its two surviving responsibilities — sprint-summary generation and agent-run recording — onto the manual and Finish paths so nothing is silently lost. **Extracted `list_backlog_issues` (#2245):** moved the backlog planner into a small planning-only module `services/sprint_manager/backlog.py` and repointed `routers/sprint_run.py` at it — the last live (non-subprocess) import from the dashboard into the orchestrator, which is what made the orchestrator deletable. **Manual-run recorder (#2246):** `routers/logs_service.py` now opens an `agent_runs` row (`db.record_manual_run_open`) on the first tool_use of a hook-driven session and closes it (`db.record_manual_run_close`) on agent_stop, so a manually-run coder/tester session gets per-agent timing rows without the orchestrator. `agent_runs.sprint_label` is now **nullable** and a new **`session_id`** column keys the open/close pair; the SQLite migration recreates the table to drop the old `NOT NULL` constraint. **Run/reasoning views tolerate a missing sprint label (#2247):** `/runs` groups NULL-sprint manual sessions under a `"null"` sentinel key so the UI can still build valid log URLs, `get_log_path_from_db` resolves `sprint="null"` → `sprint_label IS NULL`, `agent_runs_for_sprint` now folds in manual runs for issues carrying the sprint label in the issues mirror, and a new **`GET /api/manual/live`** returns active non-sprint sessions. **Sprint summary attached to Finish (#2248):** added `_generate_finish_summary` to `routers/sprint_finish.py` to build the summary artifact at the end of a successful base-sprint Finish — loading the on-disk state file when present, else a minimal state from the GitHub issue list — replacing `sprint_run.py`'s `_read_sprint_summary_url` consumer. *(Regression: #2249 subsequently deleted `services/sprint_manager/state.py`, which this path imports, so summary generation currently no-ops — needs a follow-up.)* **Deleted the sprint orchestrator (#2249):** removed eleven modules (`sprint_manager.py`, `dispatch.py`, `gates.py`, `pipeline.py`, `worktree.py`, `worktree_pool.py`, `concurrent_scheduler.py`, `serialization.py`, `timekeeping.py`, `state.py`, `config.py`) — ~15k lines. Retained `state_machine.py`, `agent_browser_runner.py`, `reconciliation.py`, `paths.py`, `commander_paths.py`, `summary.py`, and the shared BA-model portion of `model_routing.py`. **Removed dispatch controls from the UI (#2251):** deleted `run-controls.js`, the Run/Cancel/Re-run buttons, the board move-lock overlay, and the Run-sprint button on the home page; the board still renders cards, badges, counts and dispatch-level separators, and read-only preflight warnings (unestimated / stale-estimate / missing-AC) moved to `preflight-warnings.js`. Bundle rebuilt. **Deleted dispatch-pinned tests (#2252)** and ran a **post-cut verification pass across all retained surfaces (#2253)**.

- [#2245](https://github.com/zealchaiwut/commander/issues/2245) Extract list_backlog_issues out of the dispatch pipeline — 2026-08-13
- [#2246](https://github.com/zealchaiwut/commander/issues/2246) Manual-run recorder: write agent_runs rows from hooks — 2026-08-13
- [#2247](https://github.com/zealchaiwut/commander/issues/2247) Run and reasoning views tolerate a missing sprint label — 2026-08-13
- [#2248](https://github.com/zealchaiwut/commander/issues/2248) Attach sprint-summary generation to the Finish flow — 2026-08-13
- [#2249](https://github.com/zealchaiwut/commander/issues/2249) Delete the sprint orchestrator — 2026-08-13
- [#2251](https://github.com/zealchaiwut/commander/issues/2251) Remove dispatch controls from the UI, keep status rendering — 2026-08-13
- [#2252](https://github.com/zealchaiwut/commander/issues/2252) Delete dispatch-pinned tests — 2026-08-13
- [#2253](https://github.com/zealchaiwut/commander/issues/2253) Post-cut verification pass across all retained surfaces — 2026-08-13

## Sprint 1022.3

Continued autonomous-dispatch shrink (`commander-shrink-2026-08`): removed the remaining LLM-driven conflict path and dispatch glue while keeping the operator-driven Finish flow intact. **Removed AI branch-conflict resolution, kept the Finish wizard (#2240):** deleted `routers/resolve_conflict.py` + `resolve_conflict_service.py`, dropped their `include_router` wiring from `server.py`, and removed the AI-resolve `EventSource` path from `bulk-complete-modal.js` — `POST /api/projects/{owner}/{repo_name}/resolve-branch-conflict` and its `resolve-conflict-stream` SSE now 404. The Finish wizard still completes a clean sprint and now surfaces a real merge conflict for manual resolution instead of driving an LLM resolve; bundle rebuilt and committed. **Removed dispatch glue modules (#2242):** deleted `routers/sprint_webhook_service.py` and `routers/dispatch_service.py` and removed `startup.py`'s import of the latter. This also retired the `callback_url` sprint-terminal webhook (firing, SSRF screening, and bearer-token auth); the `callback_url` request field is still accepted and validated as an http/https URL but is now a no-op. The unrelated `routers/failures.py` (Failures inbox) is untouched and the dashboard starts clean.

- [#2240](https://github.com/zealchaiwut/commander/issues/2240) Remove AI branch-conflict resolution, keep the Finish wizard — 2026-08-13
- [#2242](https://github.com/zealchaiwut/commander/issues/2242) Remove dispatch glue modules — 2026-08-13

## Sprint 1022.2

Autonomous-dispatch shrink (`commander-shrink-2026-08`): removed the unattended overnight/auto-run machinery now that the operator drives sprints by hand, deleting ~10k lines with no runtime behavior change to manual flows. **Removed the overnight sprint scheduler (#2238):** deleted `routers/scheduler.py`, `routers/scheduler_service.py`, and `services/sprint_manager/sprint_scheduler.py`, dropped the `scheduler` `include_router` wiring, and removed the scheduled-run toggle (`scheduled-run.js`) from the board UI — no route under `/api/scheduler/*` remains (now 404). **Removed post-sprint auto-dispatched agents (#2239):** deleted `services/sprint_manager/post_sprint.py`, `document_issue.py`, and `scripts/run_post_sprint.py` — the operator now runs `/rev`, `/est`, and the `documenter` agent by hand; the `documenter` agent definition and the `/rev`/`/est` slash commands still work standalone. **Removed autorun babysitting modules (#2241):** deleted eight modules that existed only to keep an unattended dispatch loop alive — `alerts.py`, `dead_letter_escalation.py`, `brief_generator.py`, `label_transitions.py`, `events.py`, `ica_preflight.py`, `api_client.py`, and `services/sprint_manager/failures.py`. The unrelated `routers/failures.py`/`routers/failures_service.py` (Failures inbox) are untouched and still load. **Removed standalone live-run viewers (#2243):** deleted `static/run_browser.html` and the `/run-browser` route (the ntfy deep-link target — now 404) plus `rerun-modal.js` and its bundle entry point; bundle rebuilt and committed.

- [#2238](https://github.com/zealchaiwut/commander/issues/2238) Remove the overnight sprint scheduler — 2026-08-13
- [#2239](https://github.com/zealchaiwut/commander/issues/2239) Remove post-sprint auto-dispatched agents — 2026-08-13
- [#2241](https://github.com/zealchaiwut/commander/issues/2241) Remove autorun babysitting modules — 2026-08-13
- [#2243](https://github.com/zealchaiwut/commander/issues/2243) Remove standalone live-run viewers — 2026-08-13

## Sprint 1021

Agent-attribution and test-isolation bug fixes plus a large dead-code and orphan-endpoint cleanup. **Manual /tester now transitions to UAT (#2230):** `scripts/finish_feature.py` gained `_apply_uat_if_manual`, which applies the UAT label via `state_machine.transition` when `COMMANDER_SPRINT_RUNNING` is unset (a manual `/tester` run); under the sprint_manager dispatch loop the env var is set and finish_feature skips the transition so the dispatch path owns it — no more tickets stuck on SIT. **pytest todo-store isolation (#2231):** a new autouse `_isolate_todo_repo` conftest fixture redirects `todo_repo`'s fallback store to `tmp_path`, preventing the suite from writing through to the production `project_todos_store.json`. **Agent identity env vars for manual sessions (#2232):** the coder and tester agent instructions now export `CLAUDE_AGENT_ROLE` and `CLAUDE_AGENT_ISSUE` up front so hook calls in a manual Claude Code session carry correct attribution. **Deleted dead Logs/Activity code (#2233):** removed the unused `activity-grouping.js`, `logs-error-badge.js`, `logs-view-controls.js` ES modules and their `index.js` wiring (still executing on every page load) plus the stale `static/project.html`, and rebuilt `bundle.js`. **Deleted advisor.py orphan and six zero-caller endpoints (#2234):** removed `services/sprint_manager/advisor.py` and six endpoints with no frontend callers — `POST /api/deploy/promote` (router deleted), `POST /api/sprint-run`, `GET .../conflict-status` (flat + deprecated nested alias), `GET .../cycle-check`, `GET .../conflicts`, and `GET .../dep-order`. Cycle/conflict/dep-order data is still returned inline by the aggregate `GET .../preflight` response. **Widened .gitignore (#2235):** patterns now catch dated/rotated variants (`*.log.*`, `*.db*`, `*.bak-*`, `*.corrupt-*`), reclaiming ~300MB of stale tracked files.

- [#2230](https://github.com/zealchaiwut/commander/issues/2230) Manual /tester run never transitions ticket to UAT — 2026-08-12
- [#2231](https://github.com/zealchaiwut/commander/issues/2231) pytest writes through to the production todo store — 2026-08-12
- [#2232](https://github.com/zealchaiwut/commander/issues/2232) Export agent role and issue env vars for manual Claude Code sessions — 2026-08-12
- [#2233](https://github.com/zealchaiwut/commander/issues/2233) Delete dead Logs/Activity code still executing on every page load — 2026-08-12
- [#2234](https://github.com/zealchaiwut/commander/issues/2234) Delete advisor.py orphan and six zero-caller endpoints — 2026-08-12
- [#2235](https://github.com/zealchaiwut/commander/issues/2235) Fix .gitignore patterns and reclaim ~300MB of stale files — 2026-08-12

## Sprint 1020.2

Test-quality follow-ups resolving the #1746 source-text tension flagged in the sprint-1020 review — no runtime code changed. **Strengthened brittle #2005 AC5 check (#2190):** `test_1922__verify_all_ac` no longer greps for individual test-function names in pytest stdout; it asserts the child suite's `returncode == 0` and echoes stdout/stderr on failure so a real regression surfaces the actual output. **Strengthened #2018 conftest comment assertion (#2189):** `test_1760_guard_has_inline_comment` now requires the `1760` `_LIVE_SERVER_TEST_MODULES` comment to carry substantive rationale keywords (`self-skip`, `BASE_URL`, or `ConnectError`) rather than merely containing a `#`; a companion `test_conftest_1760_guard_comment__2189.py` enforces the same on `conftest.py`. **Sanctioned CSS-dedup source check (#2181):** the `.fbox-table` mobile-rule deduplication test (`test_2158__fbox_mobile_rule_deduplicated.py`) is documented in CLAUDE.md as an approved source-structure exception (jsdom cannot evaluate `@media`); a parallel Node structural test (`tests/frontend/css-dedup-fbox-2158.test.mjs`) also asserts the `display: none` value. **Documented #2049 source-text exceptions (#2162):** the deletion-guard and inline-JS label checks in `test_2049__settled_done_formula_unification.py` now carry docstrings explaining why a source-regex check is the only viable tool (removal guard / static template-literal in non-module inline JS).

- [#2162](https://github.com/zealchaiwut/commander/issues/2162) [follow-up] #2049 test re-introduces source-text assertions for HTML label & function deletion — 2026-08-06
...
- [#1903](https://github.com/zealchaiwut/commander/issues/1903) [follow-up] Restore precise return type annotation on db.get_conn() — 2026-08-06
- [#1990](https://github.com/zealchaiwut/commander/issues/1990) [follow-up] Remove orphaned _logsFmtTokens after #1870 dead-code sweep — 2026-08-06
