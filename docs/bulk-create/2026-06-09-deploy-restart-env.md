...
---
Add a sprint-files cleanup/archive mechanism for .commander/sprints/. The directory accumulates hundreds of per-sprint JSON files; finished sprints leave stale runtime files that clutter the dir and noise up the startup-restore scan. CRITICAL: do NOT delete files that analytics depends on — keep sprint-N-status.json (per-ticket timing/cost), sprint-N-estimate.json, and sprint-N-summary-*.md in place. ARCHIVE (move, not delete — reversible) only the transient runtime files of FINISHED sprints into .commander/sprints/archive/: sprint-N-plan.json, empty/placeholder sprint-N.json (issues:0), and superseded sprint-N-state.json. A sprint counts as "finished" when it has a posted summary issue (or a summary md exists) AND no live process is running it. (1) Add a reusable cleanup function + a CLI (scripts/clean_sprint_files.py) with --dry-run that reports what would be archived. (2) Expose a dashboard maintenance action: POST /api/maintenance/sprints/cleanup?project=... that runs the archive and returns a summary {archived: [...], kept: N}, plus a button somewhere sensible (e.g. Settings or Sprint Mgmt) with a dry-run preview before confirming. (3) Quiet the startup-restore: skip the archive/ subdir and stop logging one line per skipped finished sprint (summarize as a single count). Acceptance: running the cleanup moves only transient finished-sprint files into archive/ and leaves status/estimate/summary files untouched; dry-run shows the plan first; analytics still reads its data; startup no longer logs dozens of "skipped" lines.
```
...
- **Sprint-files cleanup** must ARCHIVE, not delete, and must NOT touch
  `*-status.json` / `*-estimate.json` / `*-summary*.md` — the analytics batch
  (2026-06-09-analytics-nav-logging.md) reads those for calibration, duration,
  and cost. Always offer a `--dry-run` / preview first.

## Follow-up enhancements (from testing the shipped Deploy tab)

The deploy/restart feature shipped in sprint-55. After running it live, these
refinements came up. Paste as a `---`-separated block.
