"""Tests for issue #1749 — Fix sqlite fd leak: get_conn() never closes connections.
...
class TestAC1ConnectionClosedOnExit:
    ...
    def test_connection_unusable_after_normal_exit(self, tmp_path, monkeypatch):
        """The yielded connection is actually closed: using it after exit raises ProgrammingError."""
        db = _reload_db(monkeypatch, tmp_path)
        conn = None
    ...
    def test_connection_unusable_after_exception_exit(self, tmp_path, monkeypatch):
        """Connection is closed even when the with-block raises — ProgrammingError on reuse."""
        db = _reload_db(monkeypatch, tmp_path)
        conn = None
        try:
            with db.get_conn() as c:
...
class TestAC2ExistingCallSitesUnchanged:
    ...
    def test_commit_and_rollback_not_broken(self, tmp_path, monkeypatch):
        """Commit persists; subsequent with-block reads back committed data."""
        db = _reload_db(monkeypatch, tmp_path)
        db.init_db()
        ...
        with db.get_conn() as conn:
            conn.execute(
                ...
                "VALUES (?, ?, ?, ?, ?)",
                ("ac2_test", "test_agent", "/tmp", "2026-01-01T00:00:00", "2026-01-01T00:00:00"),
            )
            conn.commit()

        with db.get_conn() as conn:
            row = conn.execute(
                "SELECT session_id FROM agents WHERE session_id = ?", ("ac2_test",)
            ).fetchone()
            assert row is not None
            assert row["session_id"] == "ac2_test"
