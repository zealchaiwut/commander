...
export function _smgmtRender(data) {
  ...
  const runningBanners = orderedLabels
    .filter(label => _smgmtRunningLabels.has(label))
    .map(label => _smgmtRunningBoardBannerHtml(label, bySprint[label] || []))
    .join('');
...
export function _smgmtCardHtml(label, n, tickets, outcome, isNext, parent, finished) {
  ...
  // Running sprints render as a board-top banner (hotfix 0612); full live UI is on Running tab.
  if (isRunning) {
    return '';
  }
  ...
  // Re-run Sprint button: always visible on non-running cards per issue #512
  ...
  // Action button: post-run cards show Re-run only; planning cards show Run Sprint
  let actionBtn;
  if (isPostRun) {
    actionBtn = rerunBtn;
  ...
  // Sprint summary: progress bar + goal
  const summaryDone = isOutcomeView
    ? (outcome.issues || []).filter(i => i.outcome === 'done').length
...
export function _smgmtRollupText(items) {
  ...
  if (unestimated === count) return `${countStr}, no estimates`;
...
export function _smgmtTicketRowHtml(ticket, label, elapsedSecs = null) {
  ...
  const sizePillHtml = (sizeValue && !(_cachedEst && _cachedEst.size))
    ...
    : '';
  const staleBadgeHtml = (ticket.estimate_stale && sizeLabel)
    ...
    : '';
  const reEstBtnHtml = (hasEstimate && _smgmtEstimatorAvailable && !ticket.estimate_stale)
