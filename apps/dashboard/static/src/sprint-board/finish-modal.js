...
function _fsProgressSlot() { return document.getElementById('fs-progress'); }
function _fsPreviewSlot()  { return document.getElementById('fs-content'); }
...
export async function smgmtFinishSprint(label) {
  ...
  const cancelBtn  = document.getElementById('fs-cancel-btn');
  const retryBtn   = document.getElementById('fs-retry-btn');
  if (confirmBtn) { confirmBtn.classList.remove('hidden'); confirmBtn.disabled = true; confirmBtn.textContent = 'Merge Sprint'; }
...
export async function _fsConfirm() {
  ...
  const selectedTickets = checkboxes
    .filter(c => c.checked)
    .map(c => ({ number: parseInt(c.dataset.issue, 10), title: c.dataset.title || `#${c.dataset.issue}` }));
  ...
  try {
    ...
    errEl.classList.remove('hidden');
    if (confirmBtn) { confirmBtn.classList.remove('hidden'); confirmBtn.disabled = false; confirmBtn.textContent = 'Merge Sprint'; }
  }
}
